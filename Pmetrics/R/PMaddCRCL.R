...
PMgetCRCL <- function(mdata,idCol="id",wtCol="wt",maleCol="male",ageCol="age",scrCol="scr",SI=F){
  ...
  calcCRCL <- function(temp){
    ...
    interp <- function(x,y){
      x1 <- x-1
      x2 <- x+1
      time <- tempDose$time[c(x1,x2)]
      lm1 <- lm(tempDose[c(x1,x2),y]~time)
      return(predict(lm1,data.frame(time=tempDose$time[x])))
    }
    if(length(missing[[1]])>0) tempDose[missing[[1]],scrCol] <- sapply(missing[[1]],function(x) interp(x,y=scrCol))
    if(length(missing[[2]])>0) tempDose[missing[[2]],ageCol] <- sapply(missing[[2]],function(x) interp(x,y=ageCol))
    if(length(missing[[3]])>0) tempDose[missing[[3]],wtCol] <- sapply(missing[[3]],function(x) interp(x,y=wtCol))
