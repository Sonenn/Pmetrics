...
shinyServer(function(input,output){
  ...
  makeDataControls <- function(){
    if(inherits(get(input$data),"PMcov")){
      return(list(
        ...
        selectInput("select","",choices=unique(get(input$data)$id),selected=unique(get(input$data)$id),multiple=T),
    ...
    if(inherits(get(input$data),"PMop")){
      return(list(
        radioButtons("optype","",c("Posterior Predictions" = "post","Population Predictions" = "pop"),selected="Posterior Predictions"),      
        checkboxInput("resid","Residual Plot",FALSE),
        helpText("Use Shift or CTRL (Windows) or CMD (Mac) + Click to (de)select subjects."),
        radioButtons("include","",c("Include subjects" = "yes", "Exclude subjects" = "no"),selected="Include subjects"),
        selectInput("select","",choices=unique(get(input$data)$pop1$id),selected=unique(get(input$data)$pop1$id),multiple=T),
        selectInput("outeq","Output equation:",choices=1:max(get(input$data)$pop1$outeq),selected=1)
      )) #end list
    } #end PMop
    ...
    if(inherits(get(input$data),"PMop")){
      return(list(
        ...
        checkboxGroupInput("regression","",
                           choices=c("Linear"="reg",
                                     "Lowess"="lowess",
                                     "Unity line"="ref"),selected=c("Linear","Unity line")),
        numericInput("ci","Confidence Interval",0.95,min=0.1,max=0.99,step=0.05),
        h4("Formatting Options:"),
  ...
  makeAxisControls <- function(){
    if(inherits(get(input$data),c("PMcov","PMfinal","PMop"))){
      return(list(
        h3("Axes"),
  ...
  makePMplot <- function(){
    ...
    if(identical(inherits(get(input$data),c("PMfinal","IT2B"),which=T),as.integer(1:2))){
      x <- get(input$data)
      args <- list(x=x,formula=getFormula(choices=names(get(input$data)$popMean)),cex.lab=input$cex.lab,col=getFinalPlotType()$col,
                   pch=input$pch,cex=input$cex,lwd=getFinalPlotType()$lwd,probs=getFinalprobs(),standard=input$standard,legend=input$legend,
                   grid=input$grid,xlim=getXlim(),ylim=getYlim(),xlab=getXlab(),ylab=getYlab())
      if(getFinalPlotType()$ptype=="uni"){args$formula <- NULL} #reset formula
      args <- args[which(sapply(args,function(x) length(x)>0),arr.ind=T)]
      do.call(plot.PMfinal,args)
    ...
    if(inherits(get(input$data),"PMop")){
      ...
      args <- list(x=x,include=include,exclude=exclude,type=input$optype,outeq=as.numeric(input$outeq),mult=as.numeric(input$mult),resid=input$resid,reg=reg,lowess=lowess,ref=ref,grid=input$grid,cex=input$cex,col=input$col,
  ...
  makePMstatement <- function(){
    ...
    if(inherits(get(input$data),"PMcov")){
      ...
      arglist <- c(
        ...
        FormatArg("icen",input$icen,"mean",T),
    ...
    if(inherits(get(input$data),"PMop")){
      if(length(input$include)!=0){
        if(input$include=="yes"){
          incdef <- unique(get(input$data)$pop1$id)
          excdef <- input$select
        } else {
          incdef <- input$select
          excdef <- unique(get(input$data)$pop1$id)
        }
      } else {
        incdef <- unique(get(input$data)$pop1$id)
        excdef <- input$select
      ...
      arglist <- c(
        ...
        FormatArg("include",input$select,incdef),      
        FormatArg("exclude",input$select,excdef),
        FormatArg("type",input$optype,"post",T),
